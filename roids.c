...
void ship_draw( const ship_t* ship )
{
    ...
    Draw 8 ghost ships one screen length/width away*/
    //ship 1
   draw_triangle( (ship->x+1.0) + dx, ship->y + dy,
            (ship->x+1.0) + lx, ship->y + ly,
            (ship->x+1.0) + rx, ship->y + ry,
            shipcolor );
    ...
    //ship 2
    draw_triangle( (ship->x+1.0) + dx, (ship->y+1.0) + dy,
            (ship->x+1.0) + lx, (ship->y+1.0) + ly,
            (ship->x+1.0) + rx, (ship->y+1.0) + ry,
            shipcolor );
    ...
    //3    
    draw_triangle( ship->x + dx, (ship->y+1.0) + dy,
            ship->x + lx, (ship->y+1.0) + ly,
            ship->x + rx, (ship->y+1.0) + ry,
            shipcolor );
...
void roid_draw( const roid_t* roid )
{
     ...
     */
    //1
    draw_rectangle((roid->x+1.0) - roid->width / 2.0, roid->y - roid->height / 2.0,
        (roid->x+1.0) + roid->width / 2.0, roid->y + roid->height / 2,
        roid->color);
    //2
    draw_rectangle((roid->x+1.0) - roid->width / 2.0, (roid->y+1.0) - roid->height / 2.0,
        (roid->x+1.0) + roid->width / 2.0, (roid->y+1.0) + roid->height / 2,
        roid->color);
    //3
    draw_rectangle(roid->x - roid->width / 2.0, (roid->y+1.0) - roid->height / 2.0,
        roid->x + roid->width / 2.0, (roid->y+1.0) + roid->height / 2,
        roid->color);
    //4
    draw_rectangle((roid->x-1.0) - roid->width / 2.0, (roid->y+1.0) - roid->height / 2.0,
        (roid->x-1.0) + roid->width / 2.0, (roid->y+1.0) + roid->height / 2,
...
int shot_roid_hit( const shot_t* shot, const roid_t* roid )
{
    ...
    for(i=-1; i<=1;i++)//modify shot only for corner cases, modify roid when measuring a hit
    {
      for(j=-1; j<=1;j++)
      {
      	shotX = (shot->x +i);
      	...
      	//roidX =(roidX+i);
      	//roidY=(roidY+j);
      	//modified shot position vs. unmodified roid position
      	hitOrMiss = ((shotX) >= (roidX)-(roid->width/2.0) && 
	       (shotX) <= (roidX) + (roid->width/2.0) &&
			(shotY) >= (roidY) - (roid->height/2.0) &&
			(shotY) <= (roidY) + (roid->height/2.0));
	    if(hitOrMiss==1)	
	    {
	    	return 1;
    	...
    	/*if(shotX==0 && shotY==0 || shotX == 0 && shotY==1 || shotX==1 && shotY==0 || shotX==1 || shotY==1)//|| shot->x==1 && shot->y==0
    	{	
